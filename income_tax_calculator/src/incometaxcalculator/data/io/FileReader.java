...
public abstract class FileReader {
  ...
  protected abstract String buildFieldValue(String fieldsLine);
  protected abstract boolean receiptIdExists(String values[]);
  protected int receiptIndex;
  /**
   * 
  ...
  protected String getValueOfField(String fieldsLine) throws WrongFileFormatException {
    ...
    try {
      return buildFieldValue(fieldsLine);
  ...
  protected int checkForReceipt(BufferedReader inputStream)
    ...
    while (!isEmpty(line = inputStream.readLine())) {
      ...
      if (receiptIdExists(values)) {
        int receiptId = Integer.parseInt(values[receiptIndex].trim());
        return receiptId;
      }
    }
    return -1;
